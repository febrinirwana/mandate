...
function requestBody(
  ...
  if (input instanceof Request)
    return input.json() as Promise<{ method: string; params: unknown[] }>;
  return Promise.resolve(JSON.parse(input.toString()) as { method: string; params: unknown[] });
}

function readyRpc() {
  const selectors = {
    state: encodeFunctionData({ abi: registryAbi, functionName: "getState", args: [0n] }).slice(
      0,
      10,
    ),
    ...
    owner: encodeFunctionData({ abi: registryAbi, functionName: "ownerOf", args: [1n] }).slice(
      0,
      10,
    ),
  };
  return async (input: string | URL | Request) => {
    ...
    if (
      call.to.toLowerCase() === address("1") &&
      call.data.startsWith(
        encodeFunctionData({ abi: mandateAbi, functionName: "AQUA" }).slice(0, 10),
      )
    )
      return rpcResult(
        encodeFunctionResult({ abi: mandateAbi, functionName: "AQUA", result: address("a") }),
      );
    if (call.to.toLowerCase() === address("1"))
      return rpcResult(
    ...
    if (call.data.startsWith(selectors.state))
      return rpcResult(
        encodeFunctionResult({
          abi: registryAbi,
    ...
    if (call.data.startsWith(selectors.resolver))
      return rpcResult(
        encodeFunctionResult({
          abi: registryAbi,
          functionName: "getResolver",
          result: profile.ens.resolver,
        }),
      );
    if (call.data.startsWith(selectors.owner))
      return rpcResult(
        encodeFunctionResult({
          abi: registryAbi,
          functionName: "ownerOf",
          result: profile.agent.address,
        }),
      );
    return rpcResult(
      encodeFunctionResult({
...
describe("policy readiness", () => {
  it("allows issuance only when the live app, route, and ENS binding match the trusted profile", async () => {
    ...
    ).resolves.toEqual({ kind: "READY", aqua: address("a"), maxMandateDuration: "2592000" });
  });

  it("blocks issuance when the trusted identity is no longer registered", async () => {
    const fetcher = readyRpc();
    const stateSelector = encodeFunctionData({
      abi: registryAbi,
      functionName: "getState",
      args: [0n],
    }).slice(0, 10);
    const notRegistered = async (input: string | URL | Request) => {
      ...
      if (request.method === "eth_call" && call?.data?.startsWith(stateSelector))
        return rpcResult(
          encodeFunctionResult({
            abi: registryAbi,
            functionName: "getState",
            result: { status: 0, expiry: 0n, latestOwner: address("0"), tokenId: 0n, resource: 0n },
          }),
        );
    ...
    await expect(
      checkPolicyReadiness({
        ...
        rpcUrl: "https://rpc.example",
        now: 1_800_000_000,
        fetcher: notRegistered,
      }),
    ).resolves.toEqual({
      kind: "BLOCKED",
      reason: "The configured agent identity is not registered.",
    });
  });
});
