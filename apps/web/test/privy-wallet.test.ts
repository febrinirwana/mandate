...
import { buildAuthorityCalls, submitSmartWalletCalls, type SmartWalletCall } from "../src/lib/privy-wallet";
...
const strategy = {
  version: 1 as const, maker: address("1"), agent: address("2"), ensRegistry: address("3"), ensResolver: address("4"), ensLabel: "nova", ensNode: hash("5"), tokenIn: address("6"), tokenOut: address("7"), swapTarget: address("8"), swapSelector: "0x12345678" as const, minRateNumerator: "1", minRateDenominator: "2", maxInputPerCall: "500", maxInputTotal: "1000", validAfter: "1", validUntil: "2000000000", salt: hash("9"),
};
...
describe("Privy smart wallet adapter", () => {
  it("submits the exact compiled calls as one smart-wallet request", async () => {
    ...
    await expect(submitSmartWalletCalls(client, calls)).resolves.toEqual({ kind: "SUBMITTED", txHash: hash("a") });
  ...
  it("batches exact approvals, Aqua ship, and Mandate activation", () => {
    ...
    expect(batch.map((call) => call.to)).toEqual([strategy.tokenIn, strategy.tokenOut, aqua, mandateApp]);
    const inputApproval = decodeFunctionData({ abi: parseAbi(["function approve(address spender,uint256 amount) returns (bool)"]), data: batch[0].data! }).args;
    const outputApproval = decodeFunctionData({ abi: parseAbi(["function approve(address spender,uint256 amount) returns (bool)"]), data: batch[1].data! }).args;
