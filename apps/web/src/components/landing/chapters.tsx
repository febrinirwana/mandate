...
const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    icon: ArrowUpRight,
    caption:
      "Aqua pulls from the treasury wallet, swaps through the fixed venue, and pushes output straight back. The agent never touches custody.",
    detail: (
      <div className="relative flex items-center justify-between">
        <span className="mono-data text-[0.6875rem] font-medium text-ink-2">PULL</span>
        <span
          className="absolute inset-x-8 top-1/2 h-px -translate-y-1/2 bg-rule"
          aria-hidden="true"
        />
      ...
      <article
        className={`group relative flex h-full flex-col border bg-raised p-6 transition-all duration-300 ${
          reduced
            ...
            : "border-rule hover:-translate-y-1 hover:border-ink/50 hover:shadow-[0_18px_40px_-24px_rgb(26_25_22/0.25)]"
        }`}
      >
        <div className="flex items-start justify-between">
          <span className="grid h-16 w-16 place-items-center rounded-[6px] border border-rule bg-paper transition-colors duration-300 group-hover:border-ink/30">
            <Icon
              size={30}
              strokeWidth={1.75}
              className="text-ink transition-colors duration-300 group-hover:text-accent"
            />
          </span>
          <span className="mono-data text-ink-3">{chapter.num}</span>
        </div>

        <h3 className="mt-6 border-b border-rule pb-3 text-[1.0625rem] font-medium tracking-[-0.01em]">
          {chapter.title}
        </h3>
      ...
      <div className="mx-auto max-w-[1440px] border-x border-rule px-6 py-20 md:py-28 lg:px-10">
        <Reveal>
          <div className="flex flex-wrap items-end justify-between gap-6">
            <h2 className="display max-w-[16ch] text-[clamp(2.25rem,4.6vw,4rem)]">
              How a mandate works
            </h2>
            <a
