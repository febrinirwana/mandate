...
const STATS = [
  ...
  },
  {
    ...
    label: "USDC settled · sample",
    tone: "var(--confirmed)",
  },
  {
    value: 0,
    format: (v: number) => v.toString(),
    label: "tokens held by agents",
    tone: "var(--accent)",
  },
  {
    value: 3,
    format: (v: number) => Math.round(v).toString(),
    label: "independent stop paths",
    tone: "var(--unknown)",
  },
] as const;
...
function StatCell({ stat, active }: { stat: (typeof STATS)[number]; active: boolean }) {
  ...
  return (
    <div className="pl-5">
      <div className="display text-[clamp(2rem,2.9vw,2.75rem)] leading-none tracking-[-0.04em] tabular-nums">
        {text}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[2] mx-auto flex min-h-[calc(100svh-4rem)] max-w-[1440px] flex-col border-x border-rule px-6 lg:px-10">
        <div className="grid flex-1 gap-10 lg:grid-cols-[1.08fr_0.92fr]">
          ...
          <div className="flex flex-col justify-end pb-14 pt-24 lg:pb-20">
            ...
            <p className="mt-7 flex items-center gap-2.5 text-[0.9375rem] text-ink">
              <span
                className="h-[7px] w-[7px] rotate-45 shrink-0"
        ...
        {/* mobile stats */}
        <div
          className="grid grid-cols-2 gap-y-8 pb-12 lg:hidden"
          aria-label="Protocol counters, sample data"
        >
          {STATS.map((s) => (
